...
class PledgedSecuritiesRepositoryImpl extends PledgedSecuritiesRepository{
  ...
  @override
  ResultFuture<LoanCloserResponseEntity> loanCloser(LoanCloserRequestEntity loanCloserRequestEntity) async {
    try {
      LoanCloserRequestModel loanCloserRequestModel = LoanCloserRequestModel.fromEntity(loanCloserRequestEntity);
      final response = await pledgedSecuritiesApi.loanCloser(loanCloserRequestModel);
      return DataSuccess(response.toEntity());
    } on ServerException catch (e) {
      return DataFailed(ServerFailure(e.message ?? Strings.defaultErrorMsg, 0));
    } on ApiServerException catch (e){
